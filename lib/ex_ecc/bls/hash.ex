defmodule ExEcc.Bls.Hash do
  ...
  def hkdf_expand(prk, info, length) when is_binary(prk) and is_binary(info) and is_integer(length) and length >= 0 do
  ...
  def expand_message_xmd(msg, dst, len_in_bytes) do
    ...
    b_in_bytes = 32 # :crypto.hash_info(:sha256).digest_size
    r_in_bytes = 64 # :crypto.hash_info(:sha256).block_size (not directly available, SHA256 block size is 512 bits = 64 bytes)
    ...
    z_pad = <<0::size(r_in_bytes*8)>> # binary of r_in_bytes zeros
    ...
    initial_acc = {[b_1], b_0, b_1} # {[list_of_b_i_digests], prev_b_0, prev_b_i_minus_1}
    ...
    {all_b_digests, _, _} =
      Enum.reduce(2..ell, initial_acc, fn i, {acc_b_list, acc_b_0, acc_b_i_minus_1} ->
        ...
        {[current_b_i | acc_b_list], acc_b_0, current_b_i} # Prepending, so will reverse later
      end)
